...
Ext.ux.form.ItemSelector = Ext.extend(Ext.form.Field,  {
	...
	restore: function(fromStore, toStore){
		if (!this.fromMultiselect) {
			this.fromMultiselect = new Ext.ux.form.MultiSelect(Ext.applyIf(this.multiselects[0]));	
		}
		this.fromMultiselect.store.loadData(fromStore);	
		if (this.toMultiselect) {
			this.toMultiselect.store.loadData(toStore);
		}
	},
	restoreFrom: function(fromStore){
		this.fromMultiselect.store.loadData(fromStore);	
	},
	restoreTo: function(toStore){
		this.toMultiselect.store.loadData(toStore);	
	}
});

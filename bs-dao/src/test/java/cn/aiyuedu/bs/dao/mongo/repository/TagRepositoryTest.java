...
public class TagRepositoryTest extends BaseTest {
    ...
    private final Logger logger = LoggerFactory.getLogger(TagRepositoryTest.class);
...
//        List<Tag> list = tagDao.findAll();
        ...
        if(CollectionUtils.isNotEmpty(list)){
            ...
            List<Tag> mgList = tagRepository.find(null);
            ...
            List<Tag> tempList = Lists.newArrayList();

            //向mongodb中批量插入mysql中的数据，并且记录这些数据中最大值的ID,将其设置为sequence的当前值
            for(Tag ct : list){
                if(null!=ct.getId()){
                    tagRepository.persist(ct);
            ...
            //mongodb中对应集合的名称
            String collectionName = mongoOperations.getCollectionName(Tag.class);

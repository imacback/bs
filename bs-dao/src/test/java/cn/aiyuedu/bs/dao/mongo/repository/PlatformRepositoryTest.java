...
public class PlatformRepositoryTest extends BaseTest {
    ...
    private final Logger logger = LoggerFactory.getLogger(PlatformRepositoryTest.class);
...
//    private PlatformDao platformDao;
    ...
    @Test
    public void platformDataMigration(){
        //从MySql中获取所有信息
//        List<Platform> list = platformDao.findAll();
        ...
        if(CollectionUtils.isNotEmpty(list)){
            ...
            List<Platform> mgList = platformRepository.find(null);
            if(CollectionUtils.isNotEmpty(mgList)){
                List<Integer> ids = Lists.newArrayList();
                for(Platform m : mgList){
                    ids.add(m.getId());
                }
                platformRepository.removeMulti(ids);
            }

            //用于保存批量保存后的sequence的值
            Integer sequenceNum = 0;
            //用于保存批量数据中没有ID的数据,这些数据插入时使用sequence的自动增长后的值作为ID
            List<Platform> tempList = Lists.newArrayList();

            //向mongodb中批量插入mysql中的数据，并且记录这些数据中最大值的ID,将其设置为sequence的当前值
            for(Platform ct : list){
                if(null!=ct.getId()){
                    platformRepository.persist(ct);
            ...
            //mongodb中对应集合的名称
            String collectionName = mongoOperations.getCollectionName(Platform.class);
            ...
            if(CollectionUtils.isNotEmpty(tempList)){
                ...
                for(Platform temp : tempList){
                    platformRepository.persist(temp);

 ...
 */
@Ignore
public class ComponentDataRepositoryTest extends BaseTest {

    private final Logger logger = LoggerFactory.getLogger(ComponentDataRepositoryTest.class);
...
//    @Autowired
//    private ComponentDataDao componentDataDao;
    @Autowired
    private ComponentDataRepository componentDataRepository;

    @Test
    public void componentDataDataMigration(){
        //从MySql中获取所有信息
//        List<ComponentData> list = componentDataDao.findAll();
        List<ComponentData> list = Lists.newArrayList();
        ...
        if(CollectionUtils.isNotEmpty(list)){
            ...
            List<ComponentData> mgList = componentDataRepository.find(null);
            if(CollectionUtils.isNotEmpty(mgList)){
                List<Integer> ids = Lists.newArrayList();
                for(ComponentData m : mgList){
                    ids.add(m.getId());
                }
            ...
            List<ComponentData> tempList = Lists.newArrayList();

            //向mongodb中批量插入mysql中的数据，并且记录这些数据中最大值的ID,将其设置为sequence的当前值
            for(ComponentData ct : list){
                if(null!=ct.getId()){
                    componentDataRepository.persist(ct);
            ...
            //mongodb中对应集合的名称
            String collectionName = mongoOperations.getCollectionName(ComponentData.class);
            ...
            if(CollectionUtils.isNotEmpty(tempList)){
                ...
                for(ComponentData temp : tempList){
                    componentDataRepository.persist(temp);
